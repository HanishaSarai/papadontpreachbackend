...
const theballadofbride = require('../models/theballadofbride');

// GET all youresogolden products
router.get('/', async (req, res) => {
    try {
        const goldenItems = await theballadofbride.find();
...
router.get('/:id', async (req, res) => {
    try {
        const doll = await theballadofbride.findById(req.params.id); // Use the dynamic route parameter
        if (!doll) {
...
router.post('/', async (req, res) => {
    const goldenItem = new theballadofbride({
