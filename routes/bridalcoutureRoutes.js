...
const bridalcouture = require('../models/bridalcouture');

// GET all youresogolden products
router.get('/', async (req, res) => {
    try {
        const goldenItems = await bridalcouture.find();
...
router.get('/:id', async (req, res) => {
    try {
        const doll = await bridalcouture.findById(req.params.id); // Use the dynamic route parameter
        if (!doll) {
...
router.post('/', async (req, res) => {
    const goldenItem = new bridalcouture({
