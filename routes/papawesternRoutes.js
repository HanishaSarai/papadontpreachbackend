...
router.get('/', async (req, res) => {
    try {
        const goldenItems = await papawestern.find();
...
router.get('/:id', async (req, res) => {
    try {
        const doll = await papawestern.findById(req.params.id); // Use the dynamic route parameter
        if (!doll) {
...
router.post('/', async (req, res) => {
    const goldenItem = new papawestern({
