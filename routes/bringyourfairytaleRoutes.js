...
const bringyourfairytale = require('../models/bringyourfairytale');

// GET all youresogolden products
router.get('/', async (req, res) => {
    try {
        const goldenItems = await bringyourfairytale.find();
...
router.get('/:id', async (req, res) => {
    try {
        const doll = await bringyourfairytale.findById(req.params.id); // Use the dynamic route parameter
        if (!doll) {
...
router.post('/', async (req, res) => {
    const goldenItem = new bringyourfairytale({
