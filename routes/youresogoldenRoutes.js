...
const Youresogolden = require('../models/Youresogolden');

// GET all youresogolden products
router.get('/', async (req, res) => {
    try {
        const goldenItems = await Youresogolden.find();
...
router.get('/:id', async (req, res) => {
    try {
        const doll = await Youresogolden.findById(req.params.id); // Use the dynamic route parameter
        if (!doll) {
...
router.post('/', async (req, res) => {
    const goldenItem = new Youresogolden({
